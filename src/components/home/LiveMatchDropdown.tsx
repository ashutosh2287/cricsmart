  ...
  return (
    <div className="relative">
      <button
        ...
        className="w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-overlay)] px-4 py-3 text-left transition hover:border-[var(--accent-brand)]"
      >
        <div className="flex items-center justify-between">
          <div>
            <p
              className={`text-xl font-semibold tabular-nums ${
                count > 0 ? "text-[var(--accent-brand)]" : "text-[var(--text-primary)]"
              }`}
            >
              {count}
            </p>
        ...
        <div className="absolute left-0 right-0 top-full z-50 mt-2 space-y-2">
          {matches.map((match) => (
            <Link
              ...
              className="block rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] px-4 py-3 transition hover:border-[var(--accent-brand)]"
            >
              ...
              {match.battingTeam ? (
                <p className="mt-1 text-xs font-medium text-[var(--accent-brand)]">
