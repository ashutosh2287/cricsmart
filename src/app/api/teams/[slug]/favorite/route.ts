...
import { favoriteTeam, unfavoriteTeam } from "@/lib/repositories/community.repository";
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  await favoriteTeam(access.session.userId, team.id);
...
export async function DELETE(req: NextRequest, context: RouteContext) {
  ...
  await unfavoriteTeam(access.session.userId, team.id);
  return NextResponse.json({ success: true });
}
