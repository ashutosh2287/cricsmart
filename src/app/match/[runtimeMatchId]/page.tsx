...
function TabsArea({
  ...
  const safeWinProbabilityEvents = Array.isArray(analytics?.winProbability)
    ? analytics.winProbability
    : [];
  const safeMomentumData = Array.isArray(analytics?.momentum)
    ? analytics.momentum
    : [];
  const safeInnings = Array.isArray(currentEngineState?.innings)
    ? currentEngineState.innings
    : [];

  const winProbabilityData = useMemo(
    () => calculateWinProbability(safeWinProbabilityEvents),
    [safeWinProbabilityEvents]
  );
...
// ─────────────────────────────────────────────
/**
 * ARCHITECTURE RULE — MatchInnerPage
 *
 * Every section must be wrapped in GraphErrorBoundary.
 * Graph or analytics crashes must NEVER collapse page-level rendering.
 * Commentary and Scorecard must remain visible even if graphs fail.
 *
 * Data flow:
 * replayEvents (useReplayEvents hook)
 * → selectors (pure functions, always return arrays)
 * → graph components (handle empty arrays with loading state)
 * → GraphErrorBoundary (catches any remaining crashes)
 * → page renders fully regardless of subsystem failures
 */
...
export default function MatchDetailPage({
  ...
  const normalizedReplayEvents = useMemo(
    () => dedupeReplayEvents(safeReplayEvents),
    [safeReplayEvents]
  );
